...
export const PUBLIC_ROUTE_BY_KEY = {
  home: '/',
  courses: '/courses',
  about: '/about',
  contact: '/contact',
 ...
 */
export function toPublicAppPath(href) {
  if (!href) {
    return '/';
  }
 ...
 */
export function resolvePublicMfeUrl(url, config) {
...
export function publicCoursesHref(config, query) {
...
export function isPublicMfeNavActive(urlKey, pathname) {
...
export function getPublicMfeMount(config) {
  const origin = getPublicOrigin(config);
...
export function ensurePublicMfePath(path, mount = PUBLIC_MFE_MOUNT) {
